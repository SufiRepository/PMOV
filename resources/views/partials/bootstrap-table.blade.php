...
<script nonce="{{ csrf_token() }}">
    ...
    function genericActionsFormatter(owner_name, element_name) {
        ...
        return function (value,row) {
            ...
            // testpayment task
            // if ((row.available_actions) && (row.available_actions.payment === true)) {
            //     actions += '<a href="{{ url('/') }}/' + dest + '/' + row.id + '/payment" class="btn btn-sm btn-info" data-tooltip="true" title="Clone Item"><i class="fa fa-copy" aria-hidden="true"></i><span class="sr-only">Payment</span></a>&nbsp;';
            // }
...
// add by farez 7/5/21
    function assetProjectObjFilterFormatter(value, row) {

        if ((row) && (row.company)) {
            return '<a href="{{ url('/') }}/hardware/?project_id=' + row.project.id + '"> ' + row.project.name + '</a>';
     
        }
    }
    // end 
// add by farez 2/6/2021  for clients
...
// end add
   ...
   function imageFormatter(value, row) {
    ...
    function sumDurationFormatter(data) {
        if (Array.isArray(data)) {
            var field = this.field;
            var total_sum = data.reduce(function(sum, row) {
                return (sum) + (parseFloat(row[field]) || 0);
            }, 0);
            return numberWithCommas(total_sum);
        }
        return 'not an array';
    }

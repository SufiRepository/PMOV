...
{{-- 
      ...
      {{--  new add farez 4\6 --}}
       ...
       <div class="box box-default">
          <div class="box-header with-border">
              <div class="box-heading">
                  <h2 class="box-title">{{ trans('general.project') }}</h2>
              </div>
          </div>
          <div class="box-body">
              <div class="table table-responsive">
                  ...
                  id="projectsTable"
                   ...
                   class="table table-striped snipe-table"
                   data-url="{{route('api.projects.index', ['location_id' => $location->id])}}"
                   data-export-options='{
                    "fileName": "export-locations-{{ str_slug($location->name) }}-projects-{{ date('Y-m-d') }}",
                    "ignoreColumn": ["actions","image","change","checkbox","checkincheckout","icon"]
                     }'>
         ...
         {{-- end gps --}}
          @if ($location->address!='')
           ...
           @endif
            ...
            @if ($location->gps)
            <li>{{ trans('admin/users/table.gps') }}: {!! $location->gps !!}</li>
          @endif
            {{-- end add --}}
            @if ($location->manager)

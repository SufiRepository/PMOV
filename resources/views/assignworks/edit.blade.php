@extends('layouts/edit-form', [
    'createText' => trans('admin/assignworks/general.create') ,
    'updateText' => trans('admin/assignworks/general.update'),
    'helpPosition'  => 'right',
    'helpText' => trans('help.assignworks'),
    'formAction' => (isset($item->id)) ? route('assignworks.update', ['assignwork' => $item->id]) : route('assignworks.store'),
])
  ...
  <div class="form-group {{ $errors->has('asset_tag') ? ' has-error' : '' }}">
    <label for="asset_tag" class="col-md-3 control-label">{{ trans('admin/teams/form.projectteam') }}</label>
          ...
          <div class="col-md-7 col-sm-12{{  (\App\Helpers\Helper::checkIfRequired($item, 'asset_tag')) ? ' required' : '' }}">
            ...
            <select class="form-control" name="contractor_id[]" id="asset_tag" value="{{ Request::old('asset_tag', \App\Models\Asset::autoincrement_asset()) }}" style="width: 100%">
    ...
    $(document).ready(function() {
        ...
        $(add_button).click(function(e){ //on add input button click
            ...
            if (x < max_fields) {
                ...
                box_html += '<div class="form-group"><label for="asset_tag" class="col-md-3 control-label">{{ trans('admin/teams/form.projectteam') }} ' + x + '</label>';
                box_html += '<div class="col-md-7 col-sm-12 required">';
                
                box_html += '<select class="form-control"name="contractor_id[]" value="{{ (($snipeSettings->auto_increment_prefix!='') && ($snipeSettings->auto_increment_assets=='1')) ? $snipeSettings->auto_increment_prefix : '' }}'+ auto_tag +'" data-validation="required">';
                box_html += '@foreach ($contractors as $contractor)';
                box_html += '<option value="{{ $contractor->id }}">';
                box_html += ' {{ $contractor->name }} ';
                box_html += '</option>';
                box_html += '@endforeach';
                box_html += '</select>';

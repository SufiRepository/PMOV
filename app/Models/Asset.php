...
class Asset extends Depreciable
{
    ...
    */
    ...
    //Untuk count assets,sekiranya superuser dia akan list all,and untuk admin dia akan count untuk company dia shj
...
//Untuk count assets,sekiranya superuser dia akan list all,and untuk admin dia akan count untuk project dia shj
...
// // end asset count
   ...
   */
    ...
    /**
     * Establishes the asset -> clients relationship
     *
     * @author [A. Gianotto] [<snipe@snipe.net>]
     * @since [v2.0]
     * @return \Illuminate\Database\Eloquent\Relations\Relation
  ...
  */
        ...
        return $query->where(function ($query) use ($filter) {
            foreach ($filter as $key => $search_val) {
                ...
                if ($fieldname =='project') {
                    $query->where(function ($query) use ($search_val) {
                        $query->whereHas('project', function ($query) use ($search_val) {
                            $query->where('projects.name', 'LIKE', '%' . $search_val . '%');
                        });
                    });
                }
                // end add
            ...
            if (($fieldname!='category') && ($fieldname!='model_number') && ($fieldname!='rtd_location') && ($fieldname!='location') && ($fieldname!='supplier')
                && ($fieldname!='status_label') && ($fieldname!='model') && ($fieldname!='company')&& ($fieldname!='project') && ($fieldname!='manufacturer')) {
    ...
    */
    ...
    // {
    //     return $query->leftJoin('project as project_sort', 'assets.project_id', '=', 'project_sort.id')->orderBy('project_sort.name', $order);
    // }

...
final class Company extends SnipeModel
{
    ...
    protected $rules = array(
        ...
        'parrent_company_id'    => 'integer|nullable',
     );
    ...
    */
    ...
    private static function scopeCompanyableTable($query, $column = 'id', $table_name = null )
    {
        ...
        if (!static::isFullMultipleCompanySupportEnabled() || (Auth::check() && Auth::user()->isSuperUser()) || (!Auth::check())) {
            ...
            $company_id = Auth::user()->company_id;
            $parent_company_detail = Company::select('parent_company_id')->where('id','=',$company_id);
            $table = ($table_name) ? $table_name."." : '';

            if(\Schema::hasColumn($query->getModel()->getTable(), $column)){
                return $query->where($table.$column, '=', $company_id); 
            } else {
                if($parent_company_detail == 0)
                {
                    //if company is parent, return all with the same parent_company_id
                    
                    return $query->join('users as users_comp', 'users_comp.id', 'user_id')->where('users_comp.company_id', '=', $parent_company_id); 
                }else 
                {
                    //if company is parent, return all with the same parent_company_id
                    return $query->join('users as users_comp', 'users_comp.id', 'user_id')->where('users_comp.company_id', '=', $id); 
                } 
            }
        }
    }
    ...
    {
        return $this->hasMany(Helpdesk::class, 'company_id');
    }

...
class ConsumablesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //modified SUFI , check multi tenant, if user got
        //sub_company_id or admin access, which is 0 int
        $proj_access = Auth::user()->sub_company_id;
        if($proj_access != 0){
            $consumables = Company::scopeSubCompanyables(Consumable::select('consumables.*'),"sub_company_id","consumables")
            ->with('company', 'location', 'category', 'users', 'manufacturer');
        }else{
            $consumables = Company::scopeCompanyables(Consumable::select('consumables.*'),"company_id","consumables")
            ->with('company', 'location', 'category', 'users', 'manufacturer');
        }
        ...
        if ($request->filled('project_id')) {
            $consumables->where('project_id','=',$request->input('project_id'));
        }
        ...
        $allowed_columns = ['id','name','order_number','min_amt','purchase_date','purchase_cost','company','project','category','model_number', 'item_no', 'manufacturer','location','qty','image'];

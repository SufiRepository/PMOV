...
use App\Http\Transformers\paymentsubtasksTransformer;
...
class PaymentSubtasksController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $paymentsubtasks = PaymentSubtask::select('paymentsubtasks.*');

        if ($request->filled('search')) {
            $paymentsubtasks = $paymentsubtasks->TextSearch($request->input('search'));
        }

        if ($request->filled('project_id')) {
            $paymentsubtasks->where('project_id','=',$request->input('project_id'));
        }

        if ($request->filled('user_id')) {
            $paymentsubtasks->where('user_id','=',$request->input('user_id'));
        }

        if ($request->filled('subtask_id')) {
            $paymentsubtasks->where('subtask_id','=',$request->input('subtask_id'));
        }

        // Set the offset to the API call's offset, unless the offset is higher than the actual count of items in which
        // case we override with the actual count, so we should return 0 items.
        $offset = (($paymentsubtasks) && ($request->get('offset') > $paymentsubtasks->count())) ? $paymentsubtasks->count() : $request->get('offset', 0);
        ...
        $total = $paymentsubtasks->count();
        $paymentsubtasks = $paymentsubtasks->skip($offset)->take($limit)->get();
        return (new PaymentSubtasksTransformer)->transformPaymentSubtasks($paymentsubtasks, $total);
    }

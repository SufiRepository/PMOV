...
class AccessoriesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $allowed_columns = ['id','name','model_number','eol','notes','created_at','min_amt','company_id','project_id'];

        //$accessories = Accessory::with('category', 'company', 'manufacturer', 'users', 'location');
        //$accessories ->scopeOrder

        //modified SUFI , check multi tenant, if user got
        //sub_company_id or admin access, which is 0 int
        $accessory_access_check = Auth::user()->sub_company_id;
        if($accessory_access_check != 0){
            $accessories = Company::scopeSubCompanyables(Accessory::select('accessories.*'),"sub_company_id","accessories");
        }else{
            $accessories = Company::scopeCompanyables(Accessory::select('accessories.*'),"company_id","accessories");
        }
        ...
        if ($request->filled('project_id')) {
            $accessories->where('project_id','=',$request->input('project_id'));
        }

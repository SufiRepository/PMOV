...
class ItemImporter extends Importer
{
    ...
    protected function handle($row)
    {
        ...
        $item_typeproject = $this->findCsvMatch($row, "typeproject");
        if ($this->shouldUpdateField($item_typeproject)) {
            $this->item['typeproject_id'] = $this->createOrFetchTypeproject($item_typeproject);
        }
    ...
    */
    ...
    /**
     * Fetch an existing typeproject or create new if it doesn't exist
     *
     * @author Daniel Melzter
     * @since 3.0
     * @param $row array
     * @return Typeproject
     */
    public function createOrFetchTypeproject($item_typeproject)
    {
        if (empty($item_typeproject)) {
            $item_typeproject='Unknown';
        }

        $typeproject = Typeproject::where(['name' => $item_typeproject ])->first();

        if ($typeproject) {
            $this->log('typeproject ' . $item_typeproject . ' already exists');
            return $typeproject->id;
        }

        $typeproject = new Typeproject();
        $typeproject->name = $item_typeproject;
        $typeproject->user_id = $this->user_id;

        if ($typeproject->save()) {
            $this->log('Typeproject ' . $item_typeproject . ' was created');
            return $typeproject->id;
        }
        $this->logError($typeproject, 'Typeproject');
        return null;
    }
}
